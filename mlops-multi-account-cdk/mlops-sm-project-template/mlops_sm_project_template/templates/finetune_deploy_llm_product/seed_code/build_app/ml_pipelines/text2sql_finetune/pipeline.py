...
def get_pipeline(
    ...
    processing_instance_type="ml.g4dn.xlarge",  # small gpu instance for data preprocessing (HuggingFaceProcessor requires GPU instance)
    ...
    acc_score_threshold = ParameterFloat(
        name="AccuracyScoreThreshold", default_value=0.0
    )  # Auto approve for test

    hf_model_id = ParameterString(
        name="HuggingFaceModel", default_value="codellama/CodeLlama-7b-hf"
    )

    hf_dataset_name = ParameterString(
        name="HuggingFaceDataset", default_value="philikai/Spider-SQL-LLAMA2_train"
    )
    ...
    hyperparameters = {
        ...
        "epochs": 2,  # number of training epochs --> 1 is for fast testing. Can be exposed as pipeline parameter for example.
    ...
    # The evaluate.py defines several parameters as input args. We are only passing the --dry-run parameter here as an example.
    # If you want to be able to change the other parameters at runtime, please implement them as SageMaker pipeline paramets analougously to the
    # --dry-run parameter.
