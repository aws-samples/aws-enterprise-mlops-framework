...
def preprocess(args):
    ...
    data_processor_training = CodeLlamaDataProcessor(
        dataset=dataset["train"], is_training=True
    )
    dataset_processor_test = CodeLlamaDataProcessor(
        dataset=dataset["validation"], is_training=False
    )
    ...
    if (
        ...
        logger.info(
            "Dry run, only processing a couple of examples for testing and demonstration. If this is not intended, please set the flag dry_run to False."
        )
        dataset_train = dataset_train.select(range(12))
        dataset_test = dataset_test.select(range(8))

    logger.info(
        f"Writing out datasets to {args.train_data_path} and {args.test_data_path}"
    )
