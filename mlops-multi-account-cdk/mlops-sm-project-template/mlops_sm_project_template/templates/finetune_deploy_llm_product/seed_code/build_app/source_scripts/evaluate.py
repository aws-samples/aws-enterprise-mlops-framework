...
def normalise_string(s) -> str:
    """
    Normalise string using pre-defined rules to allow for easier evaluation. For example, if the prediction only differs in letter case or spaces.
    Args:
...
def evaluate_model(args):
    ...
    with tarfile.open(
        name=os.path.join(args.model_dir, "model.tar.gz"), mode="r:gz"
    ) as tar_file:
    ...
    logger.info(f"Successfully loaded the model and tokenizer")
    ...
    cleaned_predictions = [
        clean_prediction(prediction["generated_text"]) for prediction in predictions
    ]
    ...
    eval_report_dict = {
        "metrics": {"accuracy": {"value": accuracy, "standard_deviation": "NaN"}}
    }

    os.makedirs(args.output_dir, exist_ok=True)
    logger.info(
        f"Writing out evaluation report with accuracy score: {accuracy} for a total number of samples of {len(prediction_result)}."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--do_sample", default=True, help="Whether to use sampling for generation."
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        "--top_k", type=int, default=50, help="Value of top-k sampling."
    )
    parser.add_argument(
        "--top_p", type=float, default=0.95, help="Value of top-p sampling."
    )
    parser.add_argument(
        "--repetition_penalty",
        type=float,
